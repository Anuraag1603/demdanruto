...
static UINT8 const DISPLAY_MODE        = 0b10010100; // Text on, Graphics off, Curson off, Blink off
// 100101XX
// 1001XX11
...
static UINT8 const DISPLAY_OFF         = 0b10010000;

static UINT8 const COLUMN_SIZE         = 16;


static UINT16 const Timer_Ch5_Delay    = 65535;     // ~20ms
...
void LCD_OutInteger(UINT16 const data)
{
  ...
  if (number >= 10000)
  {
    tenThousand = (UINT8)(number / 1000);
    number -= (tenThousand * 1000);
    tenThousand |= 0x30;
  }
  if (number >= 1000) 
  {

...
typedef struct ecb
{
  UINT16 count;        // Count (when event is a semaphore)
  UINT16 waitList;     // List of threads waiting for event
} OS_EVENT;
...
typedef struct tcb
{
  ...
  UINT8 priority;      // Thread priority (0 = highest)
  OS_STATE state;        // Thread state
  UINT16 delay;        // Number of ticks to delay or timeout
  struct tcb *next;    // Pointer to next TCB in TCB list
  struct tcb *prev;    // Pointer to previous TCB in TCB list
  OS_EVENT *pEvent;    // Pointer to event control block
} TCB;
...
// OS_Init
// 
// Initializes the RTOS and must be called
// prior to calling OS_Start(),
...
OS_EVENT *OS_SemaphoreCreate(UINT16 value);
...
OS_ERROR OS_SemaphoreSignal(OS_EVENT *pEvent);
...
OS_ERROR OS_SemaphoreWait(OS_EVENT *pEvent, UINT16 timeout);

// ----------------------------------------
// OS_Start
// 
// Starts the RTOS multithreading.
//
// Input:
//   none
// Output:
//   none
// Conditions:
//   Must be called prior to calling OS_Start().
//   OS_Start() should only be called once by your
//   application code. If you do call OS_Start()
//   more than once, it will not do anything on the
//   second and subsequent calls. OS_Start() will
...
OS_ERROR OS_ThreadDelete(UINT8 priority);
...
void OS_TimeDelay(UINT16 ticks);
...
#define OS_EnterCritical() { asm sei; }
#define OS_ExitCritical()  { asm cli; }
